...
// necessary classes
require_once('class.nusoap_soapclient.php');
...
//		}
    ...
    if (
      ...
      $this->setError("Unsupported HTTP response status $http_status $http_reason (nusoap_soapclient->response has contents of the response)");
      return false;
    }
...
/**
 *
 * nusoap_soapclient higher level class for easy usage.
 *
 * usage:
 *
 * // instantiate client with server info
 * $nusoap_soapclient = new nusoap_soapclient( string path [ ,boolean wsdl] );
 *
 * // call method, get results
 * echo $nusoap_soapclient->call( string methodname [ ,array parameters] );
 *
 * // bye bye client
 * unset($nusoap_soapclient);
 *
 * @author   Dietrich Ayala <dietrich@ganx4.com>
 * @version  $Id: nusoap.php,v 1.94 2005/08/04 01:27:42 snichol Exp $
 * @access   public
...
class nusoap_soapclient extends nusoap_base  {
   ...
   * @access   public
   */
  function nusoap_soapclient($endpoint,$wsdl = false,$proxyhost = false,$proxyport = false,$proxyusername = false, $proxypassword = false, $timeout = 0, $response_timeout = 120){
    parent::nusoap_base();
  ...
  function _getProxyClassCode($r) {
    ...
    $evalStr = 'class soap_proxy_'.$r.' extends nusoap_soapclient {
	'.$evalStr.'
}';
    return $evalStr;
  }
